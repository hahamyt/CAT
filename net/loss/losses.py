...
from net.loss.ghm_loss import GHMR_Loss, GHMC_Loss
...
def encoder_loss(gt_label, pred_score):
    loss_cls = GHMC_Loss()
    losc = loss_cls.calc(gt_label, pred_score)
    return losc

def bbreg_loss(gt_label, pred_score):
    loss_cls = GHMR_Loss()
    losc = loss_cls.calc(gt_label, pred_score)
    return losc
...
def fb_loss(gt_label, pred_label, pred_bb, gt_bb):
    # use GHMR loss
    loss_cls = GHMR_Loss()  # GHMC_Loss()# nn.MSELoss().cuda()
    loss_bbox = GHMR_Loss()
    # calculate the classification and bbreg error seprately
    losc = loss_cls.calc(gt_label, pred_label)
    losb = loss_bbox.calc(pred_bb, gt_bb)
    # weighted the output
    loss = losc * 1 + losb * 0
    return loss
